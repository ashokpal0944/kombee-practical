...
<div class="modal fade" id="add_role_modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<form id="addRoleModal"  action="{{ url('role-add') }}" method="post">
				@csrf
...
<script>
	$('#addRoleModal').submit(function(event) {
		...
		$.ajax({
			...
			success: function(res) {
				$('#addRoleModal').find('button').prop('disabled', false);
				$('#addRoleModal').find('button.spin-button').removeClass('loading').html('Save');
				if (res.status == "error") {
